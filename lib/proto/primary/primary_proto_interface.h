...
void primary_proto_interface_deserialize(primary::Pack* pack, network_enums* net_enums, network_signals* net_signals, network_strings* net_strings, uint64_t resample_us) {
    ...
    for(int i = 0; i < pack->hv_feedback_status_size(); i++){
		...
		(*net_enums)["HV_FEEDBACK_STATUS"]["fans_override"].push(pack->hv_feedback_status(i).fans_override());
		primary_hv_feedback_status_fans_override_enum_to_string((primary_hv_feedback_status_fans_override)pack->hv_feedback_status(i).fans_override(), buffer);
		(*net_strings)["HV_FEEDBACK_STATUS"]["fans_override"].push(buffer);
		(*net_signals)["HV_FEEDBACK_STATUS"]["fans_speed"].push(pack->hv_feedback_status(i).fans_speed());
    ...
    for(int i = 0; i < pack->lv_feedback_ts_voltage_size(); i++){
		...
		(*net_signals)["LV_FEEDBACK_TS_VOLTAGE"]["imd"].push(pack->lv_feedback_ts_voltage(i).imd());
    ...
    for(int i = 0; i < pack->ecu_inverter_status_size(); i++){
		...
		(*net_enums)["ECU_INVERTER_STATUS"]["inverter_l"].push(pack->ecu_inverter_status(i).inverter_l());
		primary_ecu_inverter_status_inverter_l_enum_to_string((primary_ecu_inverter_status_inverter_l)pack->ecu_inverter_status(i).inverter_l(), buffer);
		(*net_strings)["ECU_INVERTER_STATUS"]["inverter_l"].push(buffer);
		(*net_enums)["ECU_INVERTER_STATUS"]["inverter_r"].push(pack->ecu_inverter_status(i).inverter_r());
		primary_ecu_inverter_status_inverter_r_enum_to_string((primary_ecu_inverter_status_inverter_r)pack->ecu_inverter_status(i).inverter_r(), buffer);
		(*net_strings)["ECU_INVERTER_STATUS"]["inverter_r"].push(buffer);
    ...
    for(int i = 0; i < pack->ecu_control_status_size(); i++){
		...
		(*net_signals)["ECU_CONTROL_STATUS"]["control_state_todo"].push(pack->ecu_control_status(i).control_state_todo());
...
void primary_proto_interface_serialize_from_id(canlib_message_id id, primary::Pack* pack, device_t* device) {
    ...
    switch(id) {
        ...
        case 1544: {
            primary_hv_feedback_status_converted_t* msg = (primary_hv_feedback_status_converted_t*)(device->message);
            primary::HV_FEEDBACK_STATUS* proto_msg = pack->add_hv_feedback_status();
			proto_msg->set_fans_override((primary::primary_hv_feedback_status_fans_override)msg->fans_override);
			proto_msg->set_fans_speed(msg->fans_speed);
        ...
        case 1064: {
            primary_tlm_unix_timestamp_t* msg = (primary_tlm_unix_timestamp_t*)(device->message);
            primary::TLM_UNIX_TIMESTAMP* proto_msg = pack->add_tlm_unix_timestamp();
        ...
        case 1072: {
            primary_tlm_lap_time_converted_t* msg = (primary_tlm_lap_time_converted_t*)(device->message);
            primary::TLM_LAP_TIME* proto_msg = pack->add_tlm_lap_time();
        ...
        case 1080: {
            primary_tlm_laps_stats_converted_t* msg = (primary_tlm_laps_stats_converted_t*)(device->message);
            primary::TLM_LAPS_STATS* proto_msg = pack->add_tlm_laps_stats();
        ...
        case 1344: {
			...
			proto_msg->set_inverter_l((primary::primary_ecu_inverter_status_inverter_l)msg->inverter_l);
			proto_msg->set_inverter_r((primary::primary_ecu_inverter_status_inverter_r)msg->inverter_r);
        ...
        case 1680: {
			...
			proto_msg->set_control_state_todo(msg->control_state_todo);
