...
int secondary_fields_string_from_id(int id, char **v, size_t fields_size, size_t string_size)
{
	...
	case 1688:
		if(2 > fields_size) return 1;
		snprintf(v[0], string_size, FRONT_AMMO_COMPRESSION_FL);
		snprintf(v[1], string_size, FRONT_AMMO_COMPRESSION_FR);

		return 0;
	case 1696:
		if(2 > fields_size) return 1;
		snprintf(v[0], string_size, REAR_AMMO_COMPRESSION_RL);
		snprintf(v[1], string_size, REAR_AMMO_COMPRESSION_RR);
...
int secondary_serialize_from_id(int id, char *s, uint8_t *data, size_t *size)
{
	...
	case 1688:
	{
		secondary_front_ammo_compression_t tmp;
		secondary_front_ammo_compression_converted_t tmp_converted;
		float r_fl;
		float r_fr;
		...
		secondary_front_ammo_compression_conversion_to_raw_struct(&tmp, &tmp_converted);
		*size = SECONDARY_FRONT_AMMO_COMPRESSION_BYTE_SIZE;
		return secondary_front_ammo_compression_pack(data, &tmp, SECONDARY_FRONT_AMMO_COMPRESSION_BYTE_SIZE);
	}
	case 1696:
	{
		secondary_rear_ammo_compression_t tmp;
		secondary_rear_ammo_compression_converted_t tmp_converted;
		float r_rl;
		float r_rr;
		...
		secondary_rear_ammo_compression_conversion_to_raw_struct(&tmp, &tmp_converted);
		*size = SECONDARY_REAR_AMMO_COMPRESSION_BYTE_SIZE;
		return secondary_rear_ammo_compression_pack(data, &tmp, SECONDARY_REAR_AMMO_COMPRESSION_BYTE_SIZE);
	}
	case 1704:
	{
...
std::vector<std::string> secondary_enum_fields_from_message(const std::string& msg_name)
{
	...
	if(msg_name == ACQUISINATOR_ERRORS) {
		ret.push_back("acquisinator_errors_acquisinatore_led_code_can_not_working");
		ret.push_back("acquisinator_errors_acquisinatore_led_code_spi_error");
		ret.push_back("acquisinator_errors_acquisinatore_led_code_read_write_flash");
		ret.push_back("acquisinator_errors_acquisinatore_led_code_flashed_firmware_with_wrong_id");
	}
	if(msg_name == LINK_DEFORMATION_DEBUG_VOLTAGES) {
		ret.push_back("wheel_id");
		ret.push_back("rod_id");
	}
	if(msg_name == LINK_DEFORMATION_FL_WHEEL) {
		ret.push_back("rod_id");
	}
	if(msg_name == LINK_DEFORMATION_FR_WHEEL) {
		ret.push_back("rod_id");
	}
	if(msg_name == LINK_DEFORMATION_RL_WHEEL) {
		ret.push_back("rod_id");
	}
	if(msg_name == LINK_DEFORMATION_RR_WHEEL) {
		ret.push_back("rod_id");
	}
	if(msg_name == DEBUG_SIGNAL_5) {
		ret.push_back("device_id");
	}
	if(msg_name == DEBUG_SIGNAL_6) {
		ret.push_back("device_id");
	}
	if(msg_name == DEBUG_SIGNAL_7) {
		ret.push_back("device_id");
	}
	if(msg_name == DEBUG_SIGNAL_8) {
		ret.push_back("device_id");
	}
